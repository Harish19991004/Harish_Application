...
class LocalCapturePipeline:
    ...
    def __init__(self, buffer_dir: str, model_dir: str, publish_caption, enable_frame_ocr: bool = True):
    ...
    def poll(self, elapsed_seconds: float) -> None:
        ...
        self._poll_frame(elapsed_seconds)
        # Process newly appended playback PCM with local Vosk recognition.
        self._poll_audio(elapsed_seconds)
    ...
    def _poll_frame(self, elapsed_seconds: float) -> None:
        ...
        if not frame_path.exists() or frame_path.stat().st_mtime <= self.last_frame_mtime:
            return
        # Record the frame version before OCR begins.
        self.last_frame_mtime = frame_path.stat().st_mtime
    ...
    def _poll_audio(self, elapsed_seconds: float) -> None:
        ...
        if audio_path.stat().st_size < self.audio_offset:
            self.audio_offset = 0
